...
{	
	// Suppression des entrées de la table de commande 
	// correspondant à  ce membre et à  cette commission
	$ID_membre = $_SESSION['gpt_ID'];
	$requete = mysqli_query($link, "delete from commande where commande_membre=$ID_membre and commande_commission=$commission;");
	
	// Pour chaque ligne de la commande contenant une quantité non nulle, 
	// insérer une entrée dans la table commande.
	$commande=$_POST['commande'];
	$i=0;
	while ($i<3*$_POST['nombre_produit'])
	{
		$produit=$commande[$i++];
		$quantite=$commande[$i++];
		$ajustement=max($commande[$i++],-$quantite); // Pour éviter des quantités commandées négatives après ajustement.
		if ($quantite>0)
		{
			$requete = mysqli_query($link, "insert into commande values ($ID_membre,$commission,$produit,$quantite,'$ajustement');");
		}
	...
	$requete = mysqli_query($link, "select * from (produits left join fournisseurs on produits.produits_fournisseur=fournisseurs.fournisseurs_ID left join commande on produits.produits_ID=commande.commande_produit) where (commande_membre=$ID_membre and produits_commission=$commission and produits_actif=1) order by fournisseurs_nom, produits_nom;");
...
{
	echo 'Pas reçu de commande via le formulaire. Merci de passer par la page de saisie de commande...';
}
?>
